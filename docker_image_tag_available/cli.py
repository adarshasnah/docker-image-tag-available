...
@click.option("--app-name", envvar='APP_NAME', prompt="Image", required=True, help="Image for docker registry")
@click.option("--image-version", envvar='IMAGE_VERSION', prompt="Version", required=True, help="Version for docker registry")
def main(registry_url, user, password, app_name, image_version):    

    try:        
        r = requests.get('%s/v2/%s/tags/list' % (registry_url, app_name), auth=(user, password))
        r.raise_for_status()
